...
#import models
import xnormodel # changed for xnor
...
class Model:
	def __init__(self, args):
		...
		self.model = getattr(xnormodel, self.net_type)( # xnormodel instead of models
		...
		self.bin_op = utils.Binop(self.model) # added for xnornet
	...
	def learning_rate(self, epoch):
		if self.dataset_train_name == 'CIFAR10':
			#* ((0.1 ** int(epoch >=10)) * (0.1 ** int(epoch >= 20)) * (0.1 ** int(epoch >= 30))
			new_lr = self.lr \
							 * ((0.1 ** int(epoch >= 150)) * (0.1 ** int(epoch >= 200)) * (0.1 ** int(epoch >= 250)) \
							 * (0.1 ** int(epoch >= 300)) * (0.1 ** int(epoch >= 350))) # (1) Felix modified this
	...
	def train(self, epoch, dataloader):
		...
		for i, (input, label) in enumerate(dataloader):
			...
			self.optimizer.zero_grad() # moved for xnor
			self.bin_op.Binarization() # added for xnor
			...
			loss.backward()
			self.bin_op.Restore() # added by xnor
			self.bin_op.UpdateBinaryGradWeight() # added by xnor
			self.optimizer.step()
	...
	def test(self, dataloader):
		...
		losses = []
		accuracies = []
		self.bin_op.Binarization() # added by xnor
		...
		with torch.no_grad():
			for i, (input, label) in enumerate(dataloader):
				...
				if self.cuda:
					...
					time_list.append((time.time() - start)*1000) # *1000 for ms
		...
		print("=> TIMED, cuda: {}, avg time: {:.2f}, fps: {:.2f}\n".format(torch.cuda.is_available(), (sum(time_list)/len(loader_test))/10, 1000/((sum(time_list)/len(loader_test))/10)))
		time_list.clear()
...
for epoch in range(init_epoch, args.nepochs, 1):
	...
	log_writer.add_scalar("Loss/Train", tr_loss, epoch)
	log_writer.add_scalar("Validation/Loss", te_loss, epoch)
	log_writer.add_scalar("Validation/Accuracy", te_acc, epoch)
...
# 	early_stopping(te_loss, model, epoch)
